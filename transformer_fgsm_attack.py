...
import time
import math
from util.utils import eval_regress, write_log, plot_results_attack
...
def fgsm(test_data, epsilon, data_grad):
    # 取出每一个x的梯度负号
    sign_data_grad = data_grad.sign()
    # 构建扰动数据
    perturbed_data = test_data + epsilon * sign_data_grad
...
if __name__ == "__main__":
    ...
    perturbed_data = fgsm(test_x, epsilon, data_grad)

    # 使用扰动数据预测结果
    perturbed_output = transformer(perturbed_data)
    test_result = torch.Tensor(0)
    truth = torch.Tensor(0)
    predicted = torch.cat((test_result, perturbed_output[-1].view(-1).cpu()), 0)
    ...
    write_log(eval_metrics, log_path, epsilon)

    # 取出288分数据，作为测试
    y_pred = predicted[:288]
    y_true = test_y[:288]
    result_pic_path = 'result/result_for_fgsm/'
    # plot_results_attack(y_pred.detach().numpy(), y_true.detach().numpy(), epsilon, result_pic_path)
    plot_results_attack(y_pred, y_true, epsilon, result_pic_path)
